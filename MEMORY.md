...
- 为验证 Copilot Pro 评审流程，补充 `build/README.md` 的构建目录项目说明；根据评审意见改为“主要存放” Wails 构建资源和打包脚本，并说明 `build/ks改动.txt` 是上位机批量 SQL HTTP 调用示例。
- 本次为文档级变更，不影响 Go 后端、React 前端、HTTP 接口或运行时配置。
